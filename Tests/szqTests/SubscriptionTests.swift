...
@Suite("SubscriptionTest")
struct SubscriptionTestSuite {
  
  let ctx = Context()
  
  @Test func testSubscribe() throws {
    let address = "ipc:///tmp/zq_test_pipe_SubscriptionTest\(#line)"
  ...
  @Test func testUnsubscribe() throws {
    let address = "ipc:///tmp/zq_test_pipe_SubscriptionTest\(#line)"
    let publisher = try! ctx.bind(type: .pub, url: address)
    let subscriber = try! ctx.connect(type: .sub, url: address)
  ...
  @Test func testMultipleSubscriptions() throws {
    let address = "ipc:///tmp/zq_test_pipe_SubscriptionTest\(#line)"
    let publisher = try! ctx.bind(type: .pub, url: address)
    let subscriber = try! ctx.connect(type: .sub, url: address)
